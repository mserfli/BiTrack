...
tracker_config = {
    "offline": True,  # Ensure offline tracking mode
    "t_miss": 10,
    "t_miss_new": 3,
    "t_hit": 2,
    "match_algorithm": "MCF",
...
def apply_tracker_online(
    ...
    """
    Applies an online tracker to the detections to create forward or backward tracks.
...
def apply_tracker_offline(
    detections: list[Detections], tracker_config: dict, backward: bool = False
) -> dict[int, tuple[list[np.ndarray], list[KittiObject3d], list[np.ndarray]]]:
    """
    Applies an offline tracker to the detections to create forward or backward tracks.
        ...
        for trk in dead_tracks:
            if trk.max_hits >= tracker.t_hit:
                if backward:
                    ...
                    objs = []
                    stdevs = []
                    for box in trk.boxes[::-1]:
                        boxes.append(box)
                    for obj in trk.objs[::-1]:
                        objs.append(obj)
                    for _ in trk.boxes[::-1]:
                        stdevs.append(
                            np.sqrt(np.diag(trk.filter.kf.P))[:, np.newaxis])
                    trajectories[trk.id] = (boxes, objs, stdevs)
                else:
                    boxes = []
                    objs = []
                    stdevs = []
...
if tracker_config["offline"]:  # Offline tracking
    def sort_trajectory_by_frame(trajectory: tuple[list[np.ndarray], list[KittiObject3d], list[np.ndarray]]) -> tuple[list[np.ndarray], list[KittiObject3d], list[np.ndarray]]:
        """
        Sorts a trajectory by frame ID (sample_id).

        Args:
            trajectory (Tuple[list[np.ndarray], list[KittiObject3d]]): 
                A tuple containing:
                - A list of bounding boxes.
                - A list of `KittiObject3d` objects.
                - A list of standard deviations.

        Returns:
            Tuple[list[np.ndarray], list[KittiObject3d]]: 
                The sorted trajectory.
        """
        boxes, objs, stdev = trajectory
        frame_ids = [obj.sample_id for obj in objs]
        sorted_indices = np.argsort(frame_ids)
        sorted_boxes = [boxes[idx] for idx in sorted_indices]
        sorted_objs = [objs[idx] for idx in sorted_indices]
        sorted_stdev = [stdev[idx] for idx in sorted_indices]
        return sorted_boxes, sorted_objs, sorted_stdev

    forward_tracks = apply_tracker_offline(
        detections[50:], tracker_config, backward=False)
    backward_tracks = apply_tracker_offline(
        detections[:-50], tracker_config, backward=True)

    # Sort backward tracks by frame ID to align with forward tracks
    aligned_backward_tracks = {
        track_id: sort_trajectory_by_frame((boxes, objs, stdev))
        for track_id, (boxes, objs, stdev) in backward_tracks.items()
    }
else:
    forward_tracks = apply_tracker_online(
        detections[50:], tracker_config, backward=False)
    backward_tracks = apply_tracker_online(
        detections[:-50], tracker_config, backward=True)
...
for idx, (track_id, (boxes, _, stddevs)) in enumerate(forward_tracks.items()):
    boxes = [box[:, np.newaxis]
             for box in boxes] if tracker_config["offline"] else boxes
    track_positions = np.array(
        [box[subidx, :] for box in boxes]).reshape(-1, len(subidx))
    track_stddev = np.array(
        [stddev[subidx, :] for stddev in stddevs]).reshape(-1, len(subidx))
    for row, variable_name in enumerate(state_names, start=1):
...
fig_tracks.update_layout(
    title=f"Tracks {', '.join(state_names)} with State Uncertainty",
